...
async function test() {
    const qrImage = await Jimp.read('https://quickchart.io/qr?text=test&size=800&margin=2');
    
    // Create new image with black background at bottom
    new Jimp(800, 950, '#0a0a0a', async (err, image) => {
        if (err) throw err;
        ...
        const titleImg = new Jimp(w1, 40, 0x00000000);
        ...
        const subImg = new Jimp(w2, 20, 0x00000000);
        ...
        image.composite(titleImg, (800 - w1) / 2, 830);
        image.composite(subImg, (800 - w2) / 2, 890);
        
        await image.writeAsync('test_out_hd.png');
        console.log('done hd');
    });
}
